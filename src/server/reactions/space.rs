...
use crate::space::on_disk::OnDiskSpace;
...
#[inline(always)]
pub fn create_space_on_disk(stream: &mut impl Stream, storage: Arc<Storage>, message: &[u8], write_buf: &mut [u8], write_offset: usize, log_buf: &mut [u8], log_buf_offset: &mut usize) -> usize {
    if message.len() < 5 {
        return write_msg(stream, write_buf, write_offset, &[actions::BAD_REQUEST]);
    }
    let mut spaces;
    ...
    match storage.spaces_names.write() {
        Ok(mut spaces_names) => {
            ...
            let mut offset = *log_buf_offset;
            log_buf[offset] = actions::CREATE_SPACE_ON_DISK;
            offset += 1;
            let name_len = name.len();
            log_buf[offset] = name_len as u8;
            offset += 1;
    ...
    let l = spaces.len();
    spaces.push(
        Box::new(OnDiskSpace::new(name, size as usize))
    );
    write_msg(stream, write_buf, write_offset, &[actions::DONE, l as u8, ((l as u16) >> 8) as u8])
}
